	...
	public function edit_do(){
	    if ($this->request->isAjax()){
	        $data = [
				...
				//added by wei  on 2018-9-29 begin
...
<tbody>
    ...
    <tr>
		<td style="width:50%;">送货地址：'.$purchase['delivery_address'].'</td>
		<td style="width:50%;">送货公司：'.$purchase['delivery_company'].'</td>
    </tr>
	
	
	<tr>
		<td style="width:50%;">收货联系人：'.$purchase['receiver'].'</td>
		<td>收货人电话：'.$purchase['receivernum'].'</td>
    </tr>
...
';
	...
	public function getaddress(){
		$con_id = $this->request->param('conid',0,'intval');
		if ($con_id > 0){
		    $data = db('customers_contact')->where(['con_id' => $con_id])->find();
		    $this->success('','',['receiver' => $data['con_name'],
		        'receivernum' => $data['con_mobile']
		    ]);
		}
		$this->error('');
	}
